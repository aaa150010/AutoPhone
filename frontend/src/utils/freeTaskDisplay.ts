...
export function isHistoricalDriver(task: any): boolean {
...
export function displayTaskStatus(task: any): string {
...
export function taskPlanLabel(task: any): string {
...
export function taskPlanType(task: any): 'success' | 'warning' | 'info' | 'danger' {
...
export function taskTwofaLabel(task: any): string {
...
export function taskTwofaType(task: any): 'success' | 'warning' | 'info' {
  const status = String(task?.result?.twofa_status || '').toLowerCase()
...
export function taskPasswordLabel(task: any): string {
...
export function taskPasswordType(task: any): 'success' | 'warning' | 'info' {
  const status = String(task?.result?.password_status || '').toLowerCase()
  const flow = String(task?.result?.account_flow || '').toLowerCase()
...
export function taskRowClass({ row }: { row: any }): string {
  return ['failed', 'partial_success'].includes(String(row?.status || '')) && !isRetryResolved(row?.retry_resolved)
    ? 'is-danger-row'
    : ''
}

export function canRetryPassword(task: any): boolean {
...
export function automaticOtpRemaining(task: any, nowSeconds: number): number {
...
export function taskNeedsExistingPassword(task: any): boolean {
