...
a pure projection over a ``FreeMailboxRow`` (loosely typed as ``any`` fields
defensively) so the component stays focused on state and actions.
*/
...
export function isHistoricalMailboxDriver(row: any): boolean {
  const driver = String(row?.driver || '').trim().toLowerCase()
  return Boolean(driver) && driver !== 'protocol' && driver !== 'camoufox'
}

export function mailboxDriverLabel(row: any): string {
  const driver = String(row?.driver || '').trim().toLowerCase()
...
export function mailboxPlanLabel(row: any): string {
...
export function mailboxPlanTagType(row: any): string {
...
export function mailboxStageLabel(row: any): string {
...
export function mailboxStageType(row: any): string {
...
export function mailboxStageTooltip(row: any): string {
