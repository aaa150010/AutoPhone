/**
 * Pure helpers for runtime SMS alert deduplication and display timing.
 *
 * `createRuntimeAlertTracker` keeps a bounded, insertion-ordered set of alert
 * identities so callers only surface each alert once. Every function here is
 * side-effect free and replaces the page-local implementations it previously
 * lived beside in `composables/`.
 */
