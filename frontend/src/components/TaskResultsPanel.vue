...
<template>
  <div class="task-results">
    ...
    <el-table v-loading="props.loading" class="task-table" :data="pagedTasks" :row-key="taskRowKey" stripe height="100%" border @header-dragend="(newWidth: number, oldWidth: number, column: DragColumn) => onRunHeaderDragend(newWidth, oldWidth, column)" @selection-change="selectFreeTasks" size="small">
      ...
      <el-table-column label="密码" :width="runColWidth('密码', 64)" align="center">
        <template #default="{ row }"><el-tooltip v-if="row.run_mode === 'free_register' && row.result?.has_password" content="复制注册密码" placement="top" :show-after="250"><el-button link :icon="Key" aria-label="复制注册密码" @click="emitFreeSecret('password', [row])" /></el-tooltip><el-tooltip v-else-if="row.has_mailbox_password" content="复制邮箱密码" placement="top" :show-after="250"><el-button link :icon="Key" :loading="loadingMailboxPasswords.includes(row.task_id)" aria-label="复制邮箱密码" @click="emit('mailboxPassword', row)" /></el-tooltip><span v-else class="muted">-</span></template>
      </el-table-column>
      <el-table-column label="2FA" :width="runColWidth('2FA', 64)" align="center">
        <template #default="{ row }"><el-tooltip v-if="row.run_mode === 'free_register' && row.result?.has_totp" content="复制注册 2FA 密钥" placement="top" :show-after="250"><el-button link :icon="CopyDocument" aria-label="复制注册 2FA 密钥" @click="emitFreeSecret('totp', [row])" /></el-tooltip><el-tooltip v-else-if="row.has_totp" content="复制临时 2FA 验证码" placement="top" :show-after="250"><el-button link :icon="CopyDocument" :loading="loadingMailboxTotps.includes(row.task_id)" aria-label="复制临时 2FA 验证码" @click="emit('mailboxTotp', row)" /></el-tooltip><span v-else class="muted">-</span></template>
      </el-table-column>
      <el-table-column label="当前阶段 / 结果" :min-width="runColWidth('当前阶段 / 结果', 250)">
