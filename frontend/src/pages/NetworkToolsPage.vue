...
const config = ref<any>({ workers: 3, default_target_url: 'https://www.google.com/generate_204', default_exit_url: 'https://api.ipify.org?format=json', connect_timeout_seconds: 10, request_timeout_seconds: 30 })
const filter = ref('')
...
const filteredRows = computed(() => rows.value.filter((row) => {
  ...
  return !value || [row.masked, row.scheme, row.country, row.group, row.status, row.last_exit_ip].some((item) => String(item || '').toLowerCase().includes(value))
}))
...
async function testSub() {
  ...
  try { subscriptionResult.value = await testNetworkSubscription({ subscription_id: subscriptionId.value, target_url: config.value.default_target_url, exit_url: config.value.default_exit_url }); if (!subscriptionResult.value.tested) ElMessage.warning(subscriptionResult.value.message || '当前只能展示解析结果'); else ElMessage.success('订阅节点深度测活完成') } catch (error: any) { ElMessage.error(error?.message || '订阅节点测试失败') }
}
async function test(row: NetworkProxyRow, mode: 'quick' | 'deep') {
  selected.value = row
  try { result.value = await testNetworkProxy({ proxy_id: row.proxy_id, mode, target_url: config.value.default_target_url, exit_url: config.value.default_exit_url }); ElMessage.success(`${mode === 'quick' ? '快速' : '深度'}测活完成`); await refresh() } catch (error: any) { result.value = null; ElMessage.error(error?.message || '代理测活失败'); await refresh() }
}
