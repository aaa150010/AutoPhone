...
账密隧道行（`source_label=tunnel-auto`）由凭据模板铸造并记录出口保鲜窗口：剩余窗口不足 10 分钟的行不参与分配，到期行由目标规模维护自动铸造新 sid 替补，因此黏性时长必须大于单任务最大耗时加窗口余量。挑战废弃的隧道行在替补铸造成功后删除，手工导入行只标记废弃、永不自动删除（重新导入同一身份视为操作员确认恢复）。池目标健康数默认跟随 `concurrency`（可显式配置），可用数低于目标时自动铸造补齐，总隧道行数不超过目标的 2 倍；隧道模板未配置时以上机制全部退化为既有行为。
...
- Cloudflare、人机验证和安全挑战不解题、不伪造、不自动击打验证控件；Camoufox 链路允许在同一会话内等待挑战自然通过（上限 `protocol.security_challenge_wait_seconds`，≤60 秒），等不过不伪装成成功。挑战触发的出口立即标记 `challenge_burned` 永久退出分配，可再生的 `tunnel-auto` 隧道行由凭据模板自动铸造新 sid 替补——废弃与替补是池维护动作，不属于绕过。邮箱 OTP 请求发出前的挑战允许同邮箱换新出口加新会话自动重试（每任务 ≤`proxy_challenge_switch_limit` 次，预算独立于传输重试且不消费传输重试次数）；OTP 已发出后只记录并停止，不得重复提交邮箱。authorize 导航（注册 prelude、已有账号重登、换绑登录的授权导航，均在邮箱提交前）返回普通 403（非安全挑战页）时，允许在同一会话内原地单次重试一次，复用 403 响应刷新的 Cloudflare Cookie，不消耗 `proxy_challenge_switch_limit` 预算；重试仍 403 才按既有 `proxy_retryable` 语义换代理。30 分钟内 ≥3 个不同出口触发挑战即池级熔断：暂停批量启动、重试入队、替补铸造与挑战换线，等待操作员人工确认重置。Session 失效、网络临时错误和业务限流必须按 AutoRegister 的重试边界处理；不得把业务 429 当成可重复提交信号。
...
- 代理按 3.2 从共享健康池执行时分配，不绑定注册历史代理；429 冷却沿用 `plan_retry_after_until`；重新登录明确确认停用时同深度测活删行规则（见 3.6）。进行中的注册或测活行入队时跳过，避免重复消耗邮箱验证码。
