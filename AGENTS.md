...
- Cloudflare、人机验证和安全挑战不解题、不伪造、不自动击打验证控件；Camoufox 链路允许在同一会话内等待挑战自然通过（上限 `protocol.security_challenge_wait_seconds`，≤60 秒），等不过不伪装成成功。挑战触发的出口立即标记 `challenge_burned` 永久退出分配，可再生的 `tunnel-auto` 隧道行由凭据模板自动铸造新 sid 替补——废弃与替补是池维护动作，不属于绕过。邮箱 OTP 请求发出前的挑战允许同邮箱换新出口加新会话自动重试（每任务 ≤`proxy_challenge_switch_limit` 次，预算独立于传输重试且不消费传输重试次数）；OTP 已发出后只记录并停止，不得重复提交邮箱。30 分钟内 ≥3 个不同出口触发挑战即池级熔断：暂停批量启动、重试入队、替补铸造与挑战换线，等待操作员人工确认重置。Session 失效、网络临时错误和业务限流必须按 AutoRegister 的重试边界处理；不得把业务 429 当成可重复提交信号。
