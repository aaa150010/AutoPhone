...
账密隧道行（`source_label=tunnel-auto`）由凭据模板铸造并记录出口保鲜窗口与铸造时间（`minted_at`）：剩余窗口不足 10 分钟的行不参与分配，到期行由目标规模维护自动铸造新 sid 替补，因此黏性时长必须大于单任务最大耗时加窗口余量。隧道行替换采用逐行逻辑替换：每次成功铸造一行替补后才删除一行对应的 burned `tunnel-auto` 行，铸造失败必须保留全部 burned 行及其首次挑战证据；手工导入行只标记废弃、永不自动删除（重新导入同一身份视为操作员确认恢复）。池目标健康数默认跟随 `concurrency`（可显式配置），可用数低于目标时自动铸造补齐，总隧道行数不超过目标的 2 倍（待删除的 burned 隧道行计入可回收容量）；隧道模板未配置时以上机制全部退化为既有行为。池级维护统一收敛到 `mac_overrides/free_proxy_maintenance.py` 的 `maintain_proxy_pool()` 入口，对启动、测活、套餐查询、换绑和重试入队等所有消费者生效，并由 manager 级互斥锁串行化，禁止并发补铸超出缺口。空池分配前必须先执行一次池级维护再重新检查：熔断或网关封锁生效时不得补铸，仍为空时错误码必须区分 `free_proxy_breaker_tripped`（需操作员重置）与 `free_proxy_pool_empty`。挑战出口的熔断计数按实际观测出口 IP 优先去重，无出口 IP 时回退 `proxy_id`；无论挑战烧录发生在 worker 链路还是 bind 内部健康刷新探测，都必须经同一熔断入口计数（store 通过 `challenge_burn_observer` 回调 manager，store 本身不引用熔断器），否则网关级封锁会被绕过、设置页不会亮告警。同一网关凭据指纹下 15 分钟内 ≥3 个连续新铸 `tunnel-auto` 行触发挑战即网关疑似封锁：只停止该模板继续补铸，不影响池内其它健康出口与既有分配；池级熔断与网关封锁由同一个"人工确认并重置熔断"入口一起清除，操作员应先更换或修正网关配置再重置。

### 3.3 注册主流程与密码/2FA

- 新注册默认优先走 passwordless 邮箱 OTP；只有实际进入并提交注册密码页时才使用配置中的注册密码（默认 `Aa150010150010`）并保存密码。已有账号登录、2FA 重试和换绑优先使用已保存的真实密码；无保存密码的 passwordless 账号（Camoufox 链路）在登录密码页改走邮箱验证码登录，登录成功后按 `auto_set_password`/`auto_set_2fa` 配置补设密码与 2FA——补设前先按已保存凭据状态判断，`password_status=enabled` 或已保存 TOTP 的项直接跳过，只补设缺失项；换绑仍必须使用已保存的真实密码和已启用 TOTP。
...
- Cloudflare、人机验证和安全挑战不解题、不伪造、不自动击打验证控件；Camoufox 链路允许在同一会话内等待挑战自然通过（上限 `protocol.security_challenge_wait_seconds`，≤60 秒），等不过不伪装成成功。挑战触发的出口立即标记 `challenge_burned` 永久退出分配，可再生的 `tunnel-auto` 隧道行由凭据模板自动铸造新 sid 替补——废弃与替补是池维护动作，不属于绕过。邮箱 OTP 请求发出前的挑战允许同邮箱换新出口加新会话自动重试（每任务 ≤`proxy_challenge_switch_limit` 次，预算独立于传输重试且不消费传输重试次数）；OTP 已发出后只记录并停止，不得重复提交邮箱。authorize 导航（注册 prelude、已有账号重登、换绑登录的授权导航，均在邮箱提交前）返回普通 403（非安全挑战页）时，允许在同一会话内原地单次重试一次，复用 403 响应刷新的 Cloudflare Cookie，不消耗 `proxy_challenge_switch_limit` 预算；重试仍 403 才按既有 `proxy_retryable` 语义换代理。30 分钟内 ≥3 个不同出口触发挑战即池级熔断（不同出口按实际观测出口 IP 去重，无出口 IP 时回退 `proxy_id`）：暂停批量启动、重试入队、替补铸造与挑战换线，等待操作员人工确认重置。Session 失效、网络临时错误和业务限流必须按 AutoRegister 的重试边界处理；不得把业务 429 当成可重复提交信号。
- 2FA 与密码的自动重试是两个独立的有界预算（`twofa_auto_retry_attempts`、`password_auto_retry_attempts`，默认 2、范围 0–2），计数器互不消耗；链式顺序固定为先自动 2FA，2FA 成功后才允许自动密码补设。安全挑战、人机验证、安全策略拒绝、封号/停用、429 限流、TOTP 缺失、OTP 准备失败和 existing-login 账号一律禁止自动密码重试。自动重试子任务以 `retry_trigger=auto` 折叠挂到父任务行下展示，其失败回填父行 `retry_failure`，父行状态保持原值；手动重试子任务保持独立行、`retry_trigger=manual`。
...
- 测活不绑定账号原代理，每次执行都按 3.2 从现有健康代理池分配；空池时先按 3.2 执行一次池级维护再重新分配，仍为空按 `free_proxy_breaker_tripped`（熔断/网关封锁生效、不得补铸）与 `free_proxy_pool_empty` 区分上报；403 或安全挑战页只判定为出口/服务端策略拒绝，不得判定为账号停用；结果（状态、时间、HTTP 状态、失败结构、套餐字段）写回邮箱池行的结果记录。
- 深度测活重新登录明确确认账号停用（与接码链路同一 `account_banned` 显式判定器）时，自动将该行从 Free 邮箱池删除；删除失败则标记为不可用。注册结果与诊断日志一律保留，不得随行删除。

### 3.7 批量重查套餐

- 批量重查套餐（`POST /api/free/plan-check` 的 `mode=recheck`，邮箱池"重查套餐"按钮）对任意行可用：不论 token 是否失效、缺失或上次失败在什么节点。已保存 token 有效时直接查询，不开浏览器、不登录。
- token 失效（401）或缺失时按账号自身链路重新建立会话：protocol 账号复用纯协议重登核心（与深度测活同一 `free_protocol_relogin` 状态机，邮箱 OTP / 密码 / TOTP 分支一致）；camoufox 账号走浏览器已有账号登录态机（含 `login_totp` 两步验证分支，用已保存 TOTP 应答，无密钥即停止，不绕过）。重登后的新 access token 落库，浏览器重登后按 3.3 修正语义只补设缺失的密码/2FA。
- 代理按 3.2 从共享健康池执行时分配，不绑定注册历史代理；空池时先按 3.2 执行一次池级维护再重新分配，仍为空按 `free_proxy_breaker_tripped`（熔断/网关封锁生效、不得补铸）与 `free_proxy_pool_empty` 区分上报；429 冷却沿用 `plan_retry_after_until`；重新登录明确确认停用时同深度测活删行规则（见 3.6）。进行中的注册或测活行入队时跳过，避免重复消耗邮箱验证码。换绑按 3.5 的纯协议链路执行，同样在空池前执行一次池级维护并按上述规则区分错误码。
...
- 池级维护入口在 `mac_overrides/free_proxy_maintenance.py`，池级熔断与网关封锁状态归 `mac_overrides/free_proxy_breaker.py`；不得把补铸、封锁统计或熔断门控逻辑重新堆回兼容 facade、路由文件或各消费者服务。
...
- 池级维护与隧道替换必须保留 burned 行的首次挑战证据：补铸失败不得覆盖或删除原挑战诊断，只有对应替补铸造成功后才允许删除该 burned `tunnel-auto` 行；清理与替补事件不得盖掉首个真实业务失败（见 6）。
...
- 挑战熔断与网关封锁统计使用的网关指纹只能是脱敏短指纹（如 host:port 的散列），不得写入或导出网关地址、端口、用户名、密码、会话 sid 或 service token；熔断触发、网关封锁与人工重置都作为关联诊断事件进入同一时间线，不得覆盖首个真实挑战失败。
...
- 字体一致性（强制，避免每次返工）：同一工具栏、筛选行、表格及其相邻区域必须使用统一字号，禁止组件默认字号与页面小尺寸混用。具体：小控件、标签、按钮、下拉、表格单元格与表头统一 12px；次要说明文字（副行、单位、计数说明）用 11px；同一行/同一容器内不得同时出现 13px 与 12px，也不得让标签 14px 而相邻控件 12px。`el-alert` 标题/描述、`el-table` 表头、启动进度列表等组件默认字号（13/14px）必须用 `:deep(...)` 显式覆盖对齐到本表口径；数字输入框内部文本与相邻标签同字号。新增运营台控件或文案时，先复用页面既有 `.muted`/`summary-cell` 等已对齐样式，不得新起字号。
