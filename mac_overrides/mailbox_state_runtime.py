...
def _stable_bindings(payload: Any) -> tuple[list[tuple[int, str]], dict[str, Any] | None]:
    value = payload if isinstance(payload, Mapping) else {}
    requested = value.get("rows")
    ...
    for item in requested:
        if not isinstance(item, Mapping):
            return [], {"ok": False, "code": "mailbox_rows_invalid", "error": "设置不可用参数无效"}
