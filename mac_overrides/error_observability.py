...
NODE_LABELS = {
    ...
    "free_protocol_preflight": "Free 全协议预检",
    "free_protocol_result": "读取 Free 协议注册结果",
    "free_roxy_preflight": "RoxyBrowser 预检",
...
_ACTION_HINTS = {
    ...
    "free_protocol_result_empty": "检查全协议运行日志中最后一个已确认节点；未出现账号创建或 OAuth 回调前不会读取 Token。",
    "free_registration_incomplete": "检查邮箱验证码、账号创建和 OAuth 回调节点是否真实完成后再重跑。",
